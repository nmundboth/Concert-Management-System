...
class Assignment2:
    ...
    def connect(self, dbname: str, username: str, password: str) -> bool:
        ...
        >>> a2.connect(dbname="csc343h-mundboth", username="mundboth", password="")
        True
    ...
    def disconnect(self) -> bool:
        ...
        >>> # This example will work for you if you change the arguments as
        >>> # appropriate for your account.
        >>> a2.connect("csc343h-mundboth", "mundboth", "")
        True
        >>> a2.disconnect()
        True
        >>> a2.disconnect()
    ...
    def clock_in(self, driver_id: int, when: datetime, geo_loc: GeoLoc) -> bool:
        ...
        try:
            ...
            cur.execute(driverInDriver, driver_id)
            driver = cur.fetchall()

            conn.commit()

            if len(driver) != 0:    # driver exists in db
                driverNotClockedOut = """
                        SELECT DISTINCT driver_id
                        FROM ClockedIn c
                        WHERE c.shift_id NOT IN (
                            SELECT shift_id
                            FROM ClockedOut) NAD 
                            driver_id = 
                    """ + str(driver_id) + ';'
                cur.execute(driverNotClockedOut, driver_id)

                cOutResults = cur.fetchall()
                conn.commit()
                print(cOutResults)
                
                if len(cOutResults) == 0:      #driver not in clockedout
                    clockedIn = """
                        SELECT *
                        FROM ClockedIn;
                    """

                    cur.execute(clockedIn)
                    results = cur.fetchall() # Check for empty ClockedIn
                    conn.commit()
                    print('2')
                    
                    if len(results) == 0:   #Empty ClockedIn
                        shift_id = 1
                        print('1')
                        conn.commit()
                    else:
                        # Get last key of ClockedIn
                        max_shift_id = """
                                SELECT max(shift_id)
                                FROM ClockedIn;
                            """
                        cur.execute(max_shift_id)
                        shift_id = int(cur.fetchall()[0][0])
                        shift_id += 1 # Make newest addition last shift_id
                    ...
                    insertClockedIn = """
                            ...
                            VALUES (%s, %s, %s);
                        """
                    cur.execute(insertClockedIn, (shift_id, driver_id, when))
                    ...
                    cur.execute(insertLocation, (shift_id, when, geo_loc))
                    conn.commit()
    ...
    def pick_up(self, driver_id: int, client_id: int, when: datetime) -> bool:
        ...
        try:
            ...
            if len(driver) != 0:  # driver exists in db
                ...
                cur.execute(clientInClient, client_id)
                client = cur.fetchall()
                conn.commit()
                
                if len(client) != 0: # client exists in db
                    # check if the driver_id has a related shift_id
                    # that is in clockedin but not clockedout
                    conditionA = """
                            SELECT DISTINCT driver_id
                            FROM ClockedIn c
                            WHERE c.shift_id NOT IN (
                    ...
                    cur.execute(conditionA, driver_id)
                    aResult = cur.fetchall()
                    conn.commit()
                    
                    if len(aResult) == 1: #condition A passed
                        #storing the corresponding shift_id
                        shiftnumber = """
                        ...
                        cur.execute(shiftnumber, driver_id)
                        shift_id = str(cur.fetchall()[0][0])
                        conn.commit()

                        # check if the client_id has a related request_id
                        # that is in request but not pickup
                        conditionC= """
                        ...
                        cur.execute(conditionC, client_id)
                        cResult = cur.fetchall()
                        conn.commit()
                        
                        if len(cResult) == 1: #condition C passed
                            #storing the corresponding request_id
                            request_id = """
                                    ...
                                    WHERE request_id NOT IN (
                                        ...
                                        client_id = 
                                """ + str(client_id) + ";"
                            cur.execute(request_id, client_id)
                            request_id = str(cur.fetchall()[0][0])

                            conn.commit()

                            # Vcheck if the request_id and shift_id combination
                            # is found in dispatch
                            conditionB= """
                                   SELECT request_id
                                   FROM Dispatch
                                   WHERE request_id = %s AND shift_id = %s;
                                """
                            cur.execute(conditionB, (request_id, shift_id))
                            bResult = cur.fetchall()
                            conn.commit()
                            
                            if len(bResult) == 1: #condition B passed
                                insertPickUp = """
                                        INSERT INTO Pickup
                                        VALUES (%s, %s);
                                    """
                                cur.execute(insertPickUp, (request_id, when))
                                conn.commit()
                                pickupAdded = True
    ...
    def dispatch(self, nw: GeoLoc, se: GeoLoc, when: datetime) -> None:
        ...
        try:
            ...
            # Clients who have requested a ride and been billed
            request = """
                    CREATE TEMPORARY VIEW totalbill AS
                    SELECT request_id, sum(amount) AS total
                    FROM Billed
                    GROUP BY request_id; 
                            
                    SELECT request_id, client_id, datetime, source,
                        destination,
                        CASE WHEN total IS NULL THEN 0 ELSE total END
                        FROM Request LEFT JOIN totalbill USING (request_id) 
                        WHERE request_id NOT IN (
                        SELECT request_id
                        FROM Dispatch)
                        ORDER BY total DESC;
            ...
            # requestResult[0] = request_id, [1] = shift_id, [2] = datetime,
            # [3] = car_location
            ...
            print(requestResult)
            print('nw: ' + str(nw.longitude) + ' ' + str(nw.latitude))
            print('se: ' + str(se.longitude) + ' ' + str(se.latitude))

            inAreaRequest = set()
            
            # Finding drivers to dispatch who are closest to the clients
            # requesting a ride
            for i in range (len(requestResult)):
                #longitude  latitude
                print(str(i) + ' ' + str(requestResult[i][3].longitude) + str(requestResult[i][3].latitude))
                # if nw.longitude request[i][3].longitude, request[i][3].latitude
                if nw.longitude <= requestResult[i][3].longitude <=  se.longitude \
                        and se.latitude <= requestResult[i][3].latitude <=  nw.latitude:
                        inAreaRequest.add(requestResult[i])
            print(inAreaRequest)

            potentialdrivers = """
                    CREATE TEMPORARY VIEW driversOnShift AS
                    ...
                    CREATE TEMPORARY VIEW driversfreenow AS
                    SELECT *
                    FROM driversOnShift
                    WHERE driversOnShift.shift_id NOT IN (
                        SELECT shift_id
                        FROM dispatch) 
                        OR (SELECT count(request_id)
                            FROM Dispatch d
                            WHERE d.shift_id = driversOnShift.shift_id
                            AND request_id NOT IN (
                                SELECT request_id
                                FROM Dropoff)) = 0;    
                        
                    CREATE TEMPORARY VIEW templocation AS
                    SELECT location.shift_id, location
                    FROM Location CROSS JOIN (
                        SELECT shift_id, max(datetime)
                        FROM driversfreenow JOIN Location USING (shift_id)
                        GROUP BY shift_id) maxd 
                    WHERE location.shift_id = maxd.shift_id AND
                        location.datetime = maxd.max;
                        
                    SELECT driver_id, shift_id, location
                    FROM templocation JOIN driversfreenow USING (shift_id);
                  
                """
            # Drivers in the area
            cur.execute(potentialdrivers)
            # potentialdrivers[0] = driver_id, [1] = shift_id, [2] = location
            potentialdriversresult = cur.fetchall()
            print(potentialdriversresult)

            availDrivers = set()
            print(potentialdriversresult[0][2].longitude)
            
            # Drivers available in the area to be dispatched
            for i in range(len(potentialdriversresult)):
                if nw.longitude <= potentialdriversresult[i][2].longitude <= se.longitude \
                        and se.latitude <= potentialdriversresult[i][2].latitude <= nw.latitude:
                    availDrivers.add(potentialdriversresult[i])
...
def sample_test_function() -> None:
    ...
    try:
        ...
        connected = a2.connect("csc343h-mundboth", "mundboth", "")
        print(f"[Connected] Expected True | Got {connected}.")

        answer = a2.dispatch(GeoLoc(1, 10.0), GeoLoc(25.1, 0), datetime.now())
        ...
        # print(f"[ClockIn] Expected False | Got {clocked_in}.")
        #
        # pick_up = a2.pick_up(22222, 100, datetime.now())
